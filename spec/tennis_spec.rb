...
describe Tennis do
  ...
  describe '#start' do
    it 'verifies the end result' do
      ...
      expect(game.score.max_by { |_k, v| v }[0]).to eql(game.send(:leader))
    end
  end

  describe '#update_score' do
    it 'increment or decrement the score for the players' do
      tennis = Tennis.new({ player_one: 3, player_two: 3 })
      expect { tennis.send(:update_score) }.to change(tennis, :score)
    end
  end
end
